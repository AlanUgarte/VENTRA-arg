  ...
  return (
    <header className="sticky top-0 z-20 flex flex-wrap items-center justify-between gap-3 border-b border-border bg-card px-6 py-3.5">
      ...
      <div className="flex flex-wrap gap-2">
        ...
        {!isCashier && (
          <Pill label="Ganancia neta" value={money(overview?.totalGanancia ?? 0)} highlight="green" />
        )}
...
function Pill({ label, value, highlight }: { label: string; value: string; highlight?: 'green' | 'orange' }) {
  return (
    <div className={`flex min-w-[110px] flex-col rounded-xl border px-3 py-1.5 ${
